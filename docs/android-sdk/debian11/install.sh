...
export DROID_BUILD_TOOLS_VERSION=$(sdkmanager --list --newer | grep build-tools | grep ${ANDROID_API_LEVEL} | tail -n 1 | awk '{print $1}')
export DROID_PATCH_VERSION=$(sdkmanager --list --newer | grep patch | awk '{print $1}')
export DROID_PLATFORM_VERSION=$(sdkmanager --list --newer | grep platforms | grep ${ANDROID_API_LEVEL} | tail -n 1 | awk '{print $1}')
echo "  DROID_BUILD_TOOLS_VERSION=[${DROID_BUILD_TOOLS_VERSION}]"
echo "  DROID_PATCH_VERSION=[${DROID_PATCH_VERSION}]"
echo "  DROID_PLATFORM_VERSION=[${DROID_PLATFORM_VERSION}]"
sdkmanager --install "${DROID_BUILD_TOOLS_VERSION}" "${DROID_PATCH_VERSION}" "${DROID_PLATFORM_VERSION}"
echo "    sdkmanager --install \"${DROID_BUILD_TOOLS_VERSION}\" \"${DROID_PATCH_VERSION}\" \"${DROID_PLATFORM_VERSION}\""

# echo "sdkmanager --install \"build-tools;${ANDROID_API_LEVEL}.0.2\" \"platforms;android-${ANDROID_API_LEVEL}\""
...
# Verify tools path
alias android="${ANDROID_HOME}/cmdline-tools/latest/bin/avdmanager"
android list device

# Verify platform-tools path
# adb devices
alias analyzer="${ANDROID_HOME}/cmdline-tools/latest/bin/apkanalyzer"
# https://developer.android.com/studio/command-line/apkanalyzer?hl=fr
analyzer -h apk summary ./androind/dist/pesto.apk
analyzer -h apk file-size ./androind/dist/pesto.apk
analyzer -h apk download-size ./androind/dist/pesto.apk
analyzer -h apk features ./androind/dist/pesto.apk
analyzer -h apk compare ./androind/dist/pesto.apk ./androind/dist/pesto.previous.apk
analyzer -h files list ./androind/dist/pesto.apk
analyzer -h files cat --file /AndroidManifest.xml
}